...
class Discriminator(nn.Module):
  def __init__(self, sz):
    ...
    self.fout = nn.Linear(int(sz/4),1) # Dim 1 pr avoir une proba dans [0,1]
  def forward(self, x):
    x = F.relu(self.fc1(x))
    x = F.relu(self.fc2(x))
    x = F.relu(self.fc3(x))
    x = torch.sigmoid(self.fout(x)) # decision (proba): sigmoid pour avoir une proba
    return x
